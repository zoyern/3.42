# 3.42 — CHEAT SHEET FRACTAL
## "16 quarks. 1 règle. N dimensions. Tout émerge."
...
─────   ────────────────────     ───────────────────  ────────────────  ──────
 ...
 >>     ENTRER(ENTRER)           PUSH frame           plonger/scope     (réservé)
 ...
 Intuition : < = pousser dehors. << = pousser dehors × 2 = sortir complètement.
             ...
             Le compilateur pattern-match << → RET, >< → JMP.
```
...
─────   ────────────────────     ───────────────────  ────────────────  ──────
 ...
 !?     INVERSER(MESURER)        skip CMP             lazy/defer        (futur)
```
...
───────   ──────────────────────────────      ────────────────────────   ────────
 ...
 >> {}    ENTRER(ENTRER(CORPS))               scope imbriqué             PUSH frame
```
...
```
NIVEAU 0 : 16 quarks          → instructions CPU atomiques
NIVEAU 1 : ~60 doubles        → opérations composées (return, break, if, pipe...)
NIVEAU 2 : ~200 triples       → patterns avancés (map-reduce, assert, yield...)
...
Rien n'est hardcodé. Tout émerge de 16 verbes et 1 règle.
